...
filter_by_cliffs_delta = function(x, y) {
...
make_univariate_corr_bubbleplot = function(data, sig_gene_data, metadata, out_path = NULL) {
  ...
  for (LF in unique(sig_gene_data$fac)) {
    ...
    df_long = df_long %>% group_by(group, gene) %>% summarise(cor_exp = cor(exp, normalized_losai, method = "spearman"))
    ...
    p = ggplot(df_long, aes(x = gene_name, y = group, size = cor_exp, color = cluster)) +
    ...
    if (!is.null(out_path)) {
      ggsave(filename = paste0(out_path, "cor_LF_new", LF, ".pdf"), p, height = 4.5, width = 8)
    }
  }
  return(results)
}
