...
source('/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/scripts/CalcControlPerformance.R')
cross_predict = function(er_results, orig_x, orig_y, orig_z,
  ...
  if ( !is.null(slide_res) ) {
    ...
    sig_lfs = as.numeric(stringr::str_replace(slide_res$marginal_vars,
                                              pattern = "z|Z", replacement = ""))

    sig_lfs = paste0("Z", sig_lfs)
  ...
  if (is_classification) {
    ...
    return(list(orig_perf = orig_df, new_perf = new_df,
                ...
                new_eval = cor(new_y, new_y_pred, method = "spearman")))
  }
}
...
recluster_cross_pred = function(orig_run_path, new_run_path,
                                ...
                                lambda = 1.0, spec = 0.1, niter = 1000,...) {

  # args = list(...)

  out_path = paste0(out_path, "/delta", delta, "_lambda", lambda, "_spec", spec, "/")
  if (!dir.exists(out_path)) {
    dir.create(out_path, recursive = TRUE)
  }
  # load x,y we want to use
  orig_x = as.matrix(read.csv(paste0(orig_run_path, "/x.csv"), row.names = 1))
  orig_y = as.matrix(read.csv(paste0(orig_run_path, "/y.csv"), row.names = 1))

  # load new X, Y
  new_x = as.matrix(read.csv(paste0(new_run_path, "/x.csv"), row.names = 1))
  new_y = as.matrix(read.csv(paste0(new_run_path, "/y.csv"), row.names = 1))
  ...
  er_results = EssReg::plainER(x = orig_x,
                                       x_std = scale(orig_x),
                                       y = orig_y,
                                       std_y = FALSE,
                                       sigma = NULL,
  ...
  # SLIDEHelper::CalcControlPerformance(z_matrix = orig_z, y_path, SLIDE_res, niter = 1000,
  #                                     condition, out_path = out_path)

  CalcControlPerformance(z_matrix = orig_z, y_path, SLIDE_res, niter = 1000,
                                      condition, out_path = out_path)
  ...
  for (load_slide in c(TRUE, FALSE)) {
    if (load_slide) {
      slide_res = SLIDE_res$SLIDE_res
    ...
    perf_data = cross_predict(er_results, orig_x, orig_y, orig_z, new_x, new_y, slide_res)
    ...
    ggplot2::ggsave(filename = paste0(save_path, "crosspred_perf_plot.pdf"), plot = pl, device = "pdf",
                    height = 6, width = 5.75)
  }
}
