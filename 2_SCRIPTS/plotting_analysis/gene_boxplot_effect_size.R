...
make_gene_boxplot_effect_size = function(data, sig_gene_data, metadata, out_path = NULL) {
  ...
  for (LF in unique(sig_gene_data$fac)) {
    ...
    df_long = df_long %>% filter(gene_name %in% clust_name_colors$gene_name[1:5])

    # p = ggplot(df_long, aes(x = gene_name, y = group, size = avg_exp, color = cluster)) +
    #   geom_count() + xlab("") + ylab("") +
    #   scale_color_manual(values = unique(df_long$clust_colors)) +
    #   theme_classic() + theme(axis.text.x = element_text(angle = 90)) + #theme(text = element_text(size = 12)) +
    #   ggtitle(paste0("LF ", LF))

    p = ggpubr::ggboxplot(df_long, x = "onset", y = "exp", fill = "health", facet.by = "gene_name",
                           xlab = "",
                           scales = "free", palette = "aaas", nrow = 2, ncol = 5) +
      theme(text = element_text(size = 10)) + rotate_x_text()
      guides(fill=guide_legend(title="")) #+
    # ggpubr::stat_compare_means(label = "p.signif",
      #                                                         comparisons = list(
      #                                                           c(unique(df_long$group)[1],
      #                                                             unique(df_long$group)[2]),
      #                                                           c(unique(df_long$group)[3],
      #                                                             unique(df_long$group)[4])),
      #                                                         step.increase = 0.05, vjust = 0.5)

    results[[length(results) + 1]] = p

    if (!is.null(out_path)) {
      # ggsave(filename = paste0(out_path, "small_effsize_LF_", LF, ".pdf"), p, height = 5, width = 8)
      ggsave(filename = paste0(out_path, "small_effsize_LF_", LF, ".pdf"), p, height = 3, width = 8)
