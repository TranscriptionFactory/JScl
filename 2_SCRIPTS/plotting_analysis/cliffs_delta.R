...
plot_LF_boxplots = function(path, df_filename_RDS_pattern = "df.RDS",
  ...
  sig_genes_indiv = readRDS(check_for_file(path, file_pattern = "sig_genes.RDS"))
  all_genes_list = unlist(lapply(sig_genes_indiv, function(x) x['gene']))
  ...
  z_mat_nonsig = df[, str_to_upper(all_genes_list)]
  ...
  cliff_slide = apply(z_combined[, -1], 2, function(x) abs(cliff.delta(f = x, d = z_combined$y)$estimate))
  # cliff_slide = apply(z_combined[, -1], 2, function(x) cliff.delta(x[z_combined$y == 1], x[z_combined$y == 0], return.dm = TRUE)$estimate)
  ...
  cliff_nonsig = apply(z_nonsig[, -1], 2, function(x) abs(cliff.delta(f = x, d = z_nonsig$y)$estimate))
  # cliff_nonsig = apply(z_nonsig[, -1], 2, function(x) cliff.delta(x[z_combined$y == 1], x[z_combined$y == 0], return.dm = TRUE)$estimate)

  cliff_nonsigdf = data.frame(cliff_nonsig)
  colnames(cliff_nonsigdf)[1] = "cliff"
  cliff_nonsigdf$LF = "" #rownames(cliff_nonsigdf)
  cliff_nonsigdf$condition = "Genes"
  ...
  dir_name = paste0(path, "/fixed_LF_cliffs_delta")
