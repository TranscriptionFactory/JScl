...
CalcCliffDelta <- function(sig_z, y, comb, sig_idx = NULL){
  ...
  result_list[['all_cds']] = all_cds
  return(result_list)
}


CalcNullCliffDelta <- function(z, y, comb, sig_idx){
  rand_z = z[, -c(sig_idx)]
...
CliffDeltaPermute <- function(z, y, nrep, SLIDE_cd, mofa_cd, comb, sig_idx){
  SLIDE_pvals = c()
  mofa_pvals = c()
  ...
  for (rep in 1:nrep){
    print(rep)
    null_cd <- CalcNullCliffDelta(z, y, comb = comb, sig_idx = sig_idx)

    SLIDE_pval = CalcNullPVal(SLIDE_cd, null_cd$all_cds)
    mofa_pval = CalcNullPVal(mofa_cd, null_cd$all_cds)
    SLIDE_pvals = append(SLIDE_pvals, SLIDE_pval)
    mofa_pvals = append(mofa_pvals, mofa_pval)
    colnames(null_cds) = colnames(null_cd$all_cds)
    null_cds_list[[rep]] = null_cd

    null_cds = rbind(null_cds, null_cd$all_cds)
  }
  # null_cds = null_cds[-1, ]

  null_cds_list[['full_null_cds']] = null_cds
  return(list(SLIDE_pvals, mofa_pvals, null_cds_list))
}
