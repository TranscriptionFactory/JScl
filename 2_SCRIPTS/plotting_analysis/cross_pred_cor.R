...
#
#     # make everything except these zero in beta
#     # beta[!c(1:nrow(beta)) %in% sig_lfs] = 0
#
...
#
#   new_er_res = er_results
#
#   # new_er_res$A[!c(1:ncol(new_er_res$A)) %in% sig_lfs] = 0
#
...
cross_predict = function(er_results, orig_x, orig_y, orig_z,
  ...
  if ( !is.null(slide_res) ) {
    ...
    if (any(stringr::str_detect(slide_res$marginal_vars, pattern = "Z|z"))) {
      # we are using old results format
      sig_lfs = stringr::str_to_upper(slide_res$marginal_vars)
    } else {
  ...
  new_A = EssReg::threshA(er_results$A, 0.08, scale = T)
  er_results$A = new_A
  ...
  # # get new predicted Z
  # # new_z = EssReg::predZ(new_x_worg, er_results)
  # new_z = EssReg::predZ(scale(new_x_worg), er_results)
  #
  # # get new prediction
  # new_y_pred = new_z %*% beta
  #
  # # remake z matrix so we use the same allocation for both
  # # orig_y_pred = as.matrix(orig_z) %*% as.matrix(beta)
  # orig_z_remade = EssReg::predZ(scale(as.matrix(orig_x)), er_results)
...
cross_predict_loader = function(orig_run_path, new_run_path,
  ...
  plot_title = ifelse(is_classification, "ER Cross Prediction AUCs ",
                      "ER Cross Prediction Corr")
  ...
  if (load_slide) {
    ...
    plot_title = paste0(plot_title, "(signif. LFs only)\n")
  } else {
    slide_res = NULL
    plot_title = paste0(plot_title, "(all LFs)\n")
  ...
  ggplot2::ggsave(filename = paste0(save_path, "perf_plot.png"), plot = pl,
                  height = 5, width = 7)
}
