# Model LS adult vs peds - 50 top var genes
...
saveRDS(Var.df2, '/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/data/new_clusters/LS_LoSAI_pre.RDS')

# write.csv(Var.df2, "/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/data/VarLS_RM.csv")

check_y<-as.data.frame(data@meta.data)

check_y2<-check_y %>%
  group_by(library_id, onset) %>%
  summarise()

# check_y2$onset <-ifelse(check_y2$onset == "Adult", 1, 0)
...
resid = check_y2 %>% select(sex, onset, LoSAI.mLoSSI)

resid$sex = ifelse(resid$sex == "F", 1, 0)
resid$onset = ifelse(resid$onset == "Adult", 1, 0)
...
saveRDS(df, '/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/data/Giffin_new_annotations/LS_LoSAI.RDS')


#
# check_y2 = check_y2[match(row.names(Var.df2), row.names(check_y2)),]
#
#
# write.csv(check_y2, "/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/data/check_yLS_onset.csv", row.names = F)
#
