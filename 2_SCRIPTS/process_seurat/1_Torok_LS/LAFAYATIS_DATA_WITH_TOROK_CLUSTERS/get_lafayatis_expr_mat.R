...
load('/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/6_LAFAYATIS_DATA/2_DATA/harmonskinV6.RData')
torok_32serclust_to_lafayatis_predictions <- readRDS("/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/6_LAFAYATIS_DATA/torok_32serclust_to_lafayatis_predictions.RDS")
...
data$seurat_clusters = torok_32serclust_to_lafayatis_predictions$predicted.id

sample_id<-unique(data@meta.data$sampleID)
...
for (i in 1:length(clusters)
  ...
  for (j in 1:length(sample_id)){
    ...
    tryCatch(
      {
        tmp<-subset(data ,subset = (data@meta.data[["sampleID"]] == sample_id[j])&(seurat_clusters == clusters[i]))
...
saveRDS(Var.df2, "/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/6_LAFAYATIS_DATA/transfered_x_data.RDS")

# write.csv(Var.df2, "/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/data/VarAll_RM.csv")
#
check_y<-as.data.frame(data@meta.data) %>% group_by(sampleID, health) %>% summarise()

saveRDS(check_y, "/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/6_LAFAYATIS_DATA/transfered_y_data.RDS")
