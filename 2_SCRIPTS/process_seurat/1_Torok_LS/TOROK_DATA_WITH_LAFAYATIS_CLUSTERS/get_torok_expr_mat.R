...
data = LoadH5Seurat('/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/6_LAFAYATIS_DATA/TOROK_CONVERTED_TO_LAFAYATIS_CLUST/LS.all_pred.H5Seurat')
lafayatis_clust <- readRDS("/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/6_LAFAYATIS_DATA/TOROK_CONVERTED_TO_LAFAYATIS_CLUST/torok_anchors_in_lafayatis_predictions.RDS")
data$seurat_clusters = lafayatis_clust$predicted.id

data <- subset(x = data, subset = (health == "LS"))
...
clusters<-sort(as.numeric(unique(data$seurat_clusters)))#c(0:max(as.numeric(data$seurat_clusters)-1))
...
saveRDS(Var.df2, "/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/1_DATA/TOROK_DATA_WITH_LAFAYATIS_CLUSTERS/LS_transfered_x_nomedianfilter.RDS")
...
check_y<-as.data.frame(data@meta.data) %>% group_by(library_id, health) %>% summarise()

saveRDS(check_y, "/ix/djishnu/Aaron/3_collabs/1_Torok_Jscl_ER/ER_analysis/1_DATA/TOROK_DATA_WITH_LAFAYATIS_CLUSTERS/LS_transfered_y_nomedianfilter.RDS")
